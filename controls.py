...
def create_army(screen, inos):
    ...
    for row_number in range(number_ino_y-4):
        for ino_numer in range(number_ino_x):
            ino = Ino(screen)
            ino.x = ino_width + (ino_width * ino_numer)
            ino .y = ino_height + (ino_height * (row_number+1))
            ino.rect.x = ino.x
            ino.rect.y = ino.rect.height + (ino.rect.height * (row_number+1))
            inos.add(ino)
